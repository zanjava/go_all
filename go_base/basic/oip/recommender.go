...
import (
	"fmt"
	"go/base/basic/oip/common"
	"go/base/basic/oip/filter"
	"go/base/basic/oip/recall"
	"go/base/basic/oip/sort"
	"time"
...
func (rec *Recommender) Rec() []*common.Product {
	...
	//对召回的结果进行排序
	SortedResult := RecallSlice
	for _, sorter := range rec.Sorter {
		begin := time.Now()
		SortedResult = sorter.Sort(SortedResult)
		log.Printf("排序%s用时%dns\n", sorter.Name(), time.Since(begin).Nanoseconds())
	}
...
// 面向接口编程，推荐系统的实现可以有多种方式。每种方式都实现了recall.Recaller、sort.Sorter和filter.Filter接口。
// 这样可以让不同的召回、排序和过滤方式互相替换，便于扩展和维护。
func main() {
	rec := Recommender{
		...
		Sorter: []sort.Sorter{
			sort.RatioSorter{Tag: "ratio"},
			sort.SizeSorter{Tag: "size"},
		},
		Filters: []filter.Filter{
