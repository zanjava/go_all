...
func main1() {
	...
	_, err = ch.QueueDeclare(
		...
		true,      // delete when unused。若设为true，当所有consumer都退出后队列会自动被删除(这意味着Server要监听Consumer进程是否存活)。
	...
	//ch2, _ := conn.Channel()
	//发送消息。通过2个channel往同一个队列里面写入消息
	//produce("hello zgw", ch, "", QueueName)
	// produce("hello world", ch, "", QueueName)
	// produce("hello golang", ch2, "", QueueName)

	//发送消息。可以通过多个channel往同一个队列里面写入消息
	ch2, _ := conn.Channel()
	go Produce("hello zgw", ch, "", QueueName)
	go Produce("hello world", ch, "", QueueName)
	go Produce("hello golang", ch2, "", QueueName)

	time.Sleep(2 * time.Second) //等待消息发送完
}
